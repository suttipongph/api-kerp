...
const handler = require("../handler/assessment_trans_handler");
...
let getAssessmentTransCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastAssessmentTransCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown,getAssessmentTransCode };
