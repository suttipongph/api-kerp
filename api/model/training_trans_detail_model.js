...
const handler = require("../handler/training_trans_detail_handler");
...
let getTrainingTransDetailCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastTrainingTransDetailCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getTrainingTransDetailCode };
