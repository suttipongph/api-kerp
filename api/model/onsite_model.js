...
const handler = require("../handler/onsite_handler");
...
let getOnsiteCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastOnsiteCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getOnsiteCode };
