...
const handler = require("../handler/customer_handler");
...
let getCustomerCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastCustomerCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getCustomerCode };
