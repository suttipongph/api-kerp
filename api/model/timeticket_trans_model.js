...
const handler = require("../handler/timeticket_trans_handler");
let create = async (req) => {
  try {
    let result = await pool.query(
      handler.getSqlCreate(req.body),
      handler.getValuesCreate(req.body)
    );
...
let edit = async (req) => {
  try {
    const result = await pool.query(
      handler.getSqlEdit(req.body),
      handler.getValuesEdit(req.body)
    );
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
