...
const handler = require("../handler/worktime_trans_handler");
...
let createCopy = async (item) => {
  try {
    let result = await pool.query(
      handler.getSqlCreateItem(item),
      handler.getValuesCreateItem(item)
    );
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
...
let viewEmp = async (req) => {
  try {
    const key = req.body.employee_uuid;
    const result = await pool.query(handler.getSqlViewEmp(req), [key]);
    return result.rows;
  } catch (e) {
    throw new Error(e.message);
  }
...
module.exports = { list, create, edit, view, del, dropdown, viewEmp, createCopy };
