...
const handler = require("../handler/certificate_trans_handler");
...
let getCertificateTransCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastCertificateTransCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getCertificateTransCode };
