...
const handler = require("../handler/contact_person_handler");
...
let getContactPersonCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastContactPersonCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getContactPersonCode };
