...
const handler = require("../handler/document_status_handler");
...
let getDocumentStatusByReftype = async (ref_type, statusId) => {
  try {
    let result = await pool.query(
      handler.getDocumentStatusByReftype(ref_type, statusId)
    );
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
