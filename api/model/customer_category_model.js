...
const handler = require("../handler/customer_category_handler");
...
let getCustomerCategoryCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastCustomerCategoryCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getCustomerCategoryCode };
