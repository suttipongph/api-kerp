...
const handler = require("../handler/training_trans_handler");
...
let getTrainingTransCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastTrainingTransCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getTrainingTransCode };
