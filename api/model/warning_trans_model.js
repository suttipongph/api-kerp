...
const handler = require("../handler/warning_trans_handler");
...
let getDataWarningTransCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastWarningTransCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getDataWarningTransCode };
