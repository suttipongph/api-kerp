...
const handler = require("../handler/responsibility_device_handler");
...
let getResponsibilityDeviceCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastResponsibilityDeviceCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getResponsibilityDeviceCode };
