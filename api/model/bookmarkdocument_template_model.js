...
const handler = require("../handler/bookmarkdocument_template_handler");
...
let getBookmarkById = async (req) => {
  try {
    try {
      const key = req.body.document_template_uuid;
      const result = await pool.query(handler.getSqlView(req), [key]);
      return result.rows[0];
    } catch (e) {
      throw new Error(e.message);
...
module.exports = { list, create, edit, view, del, dropdown, getBookmarkById };
