...
const handler = require("../handler/branch_handler");
...
let getListBranchPage = async (req) => {
  try {
    let result = await pool.query(handler.getSqlListBranchPage(req));
    handler.getRoWAuth(result.rows);
    return result.rows;
  } catch (e) {
    throw new Error(e.message);
...
module.exports = { list, create, edit, view, del, dropdown, getListBranchPage };
