...
const handler = require("../handler/assets_handler");
...
let getAssetsCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastAssetsCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getAssetsCode };
