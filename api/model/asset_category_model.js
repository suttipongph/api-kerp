...
const handler = require("../handler/asset_category_handler");
...
let getAssetsCategoryCode = async (req) => {
  try {
    const result = await pool.query(handler.getSqlGetLastAssetsCategoryCode(req));
    return result.rows[0];
  } catch (e) {
    throw new Error(e.message);
  }
};
module.exports = { list, create, edit, view, del, dropdown, getAssetsCategoryCode };
