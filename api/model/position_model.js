...
const handler = require("../handler/position_handler");
...
let getFielChangeDropdown = async (req, depId) => {
  try {
    let result = await pool.query(handler.getFielChangeDropdown(req, depId));
    return handler.toDropdown(result.rows);
  } catch (e) {
    throw new Error(e.message);
  }
