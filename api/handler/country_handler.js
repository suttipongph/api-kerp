...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("country_code");
    keyList.push("country_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE country_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE country_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_country_table_dropdown " + mTools.getConditionOnly(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE country_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE country_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_country_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_country_table_item WHERE country_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.country_code);
    valuelist.push(data.country_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.country_code = req.body["country_code"];
    model.country_name = req.body["country_name"];
