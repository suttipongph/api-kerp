...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("asset_category_code");
    keyList.push("description");
    keyList.push("asset_category_name");
    keyList.push("remark");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("asset_category_uuid");
    keyList.push("asset_category_code");
    keyList.push("description");
    keyList.push("asset_category_name");
    keyList.push("remark");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO asset_category_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE asset_category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE asset_category_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_asset_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE asset_category_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE asset_category_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_asset_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_asset_category_table_item WHERE asset_category_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.asset_category_code);
    valuelist.push(data.description);
    valuelist.push(data.asset_category_name);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.asset_category_uuid);
    valuelist.push(data.asset_category_code);
    valuelist.push(data.description);
    valuelist.push(data.asset_category_name);
    valuelist.push(data.remark);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.asset_category_code = req.body["asset_category_code"];
    model.description = req.body["description"];
    model.asset_category_name = req.body["asset_category_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["asset_category_uuid"];
...
let getLabel = (model) => {
  try {
    return model["asset_category_code"] + " - " + model["asset_category_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.asset_category_uuid = req.body["asset_category_uuid"];
    model.asset_category_code = req.body["asset_category_code"];
    model.description = req.body["description"];
    model.asset_category_name = req.body["asset_category_name"];
    model.remark = req.body["remark"];
...
let getSqlGetLastAssetsCategoryCode = (req) => {
  return (
    "SELECT * FROM vw_asset_category_table_item " +
    mTools.getConditionCompanyAndBranch(req) +
    " ORDER BY asset_category_code DESC LIMIT 1"
  );
};
module.exports = {
  toDropdown,
  ...
  getSqlDropdown,
  getRoWAuth,
  getSqlGetLastAssetsCategoryCode
};
