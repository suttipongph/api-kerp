...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("province_code");
    keyList.push("province_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE province_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE province_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_province_table_dropdown " + mTools.getConditionOnly(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE province_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE province_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_province_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_province_table_item WHERE province_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.province_code);
    valuelist.push(data.province_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.province_code = req.body["province_code"];
    model.province_name = req.body["province_name"];
...
let getLabel = (model) => {
  try {
    return model["province_code"] + " - " + model["province_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
