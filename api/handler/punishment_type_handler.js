...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("punishment_type_name");
    keyList.push("is_deduct_salary");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO punishment_type_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE punishment_type_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE punishment_type_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_punishment_type_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE punishment_type_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE punishment_type_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_punishment_type_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_punishment_type_table_item WHERE punishment_type_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.punishment_type_name);
    valuelist.push(data.is_deduct_salary);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.punishment_type_name = req.body["punishment_type_name"];
    model.is_deduct_salary = req.body["is_deduct_salary"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["punishment_type_uuid"];
...
let getLabel = (model) => {
  try {
    return model["punishment_type_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
