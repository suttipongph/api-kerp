...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("sub_district_name");
    keyList.push("sub_district_code");
    keyList.push("remark");
    keyList.push("descrption");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO sub_district_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE sub_district_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE sub_district_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_sub_district_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE sub_district_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE sub_district_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_sub_district_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_sub_district_table_item WHERE sub_district_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.sub_district_name);
    valuelist.push(data.sub_district_code);
    valuelist.push(data.remark);
    valuelist.push(data.descrption);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.sub_district_name = req.body["sub_district_name"];
    model.sub_district_code = req.body["sub_district_code"];
    model.remark = req.body["remark"];
    model.descrption = req.body["descrption"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["sub_district_uuid"];
...
let getLabel = (model) => {
  try {
    return model["sub_district_code"] + " - " + model["sub_district_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
