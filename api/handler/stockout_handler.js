...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("voucher_out_id");
    keyList.push("stockout_date");
    keyList.push("stockout_time");
    keyList.push("receive_uuid");
    keyList.push("sender_uuid");
    keyList.push("ref_out");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE stockout_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE stockout_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_stockout_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE stockout_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE stockout_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_stockout_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_stockout_table_item WHERE stockout_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.voucher_out_id);
    valuelist.push(data.stockout_date);
    valuelist.push(data.stockout_time);
    valuelist.push(data.receive_uuid);
    valuelist.push(data.sender_uuid);
    valuelist.push(data.ref_out);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.stockout_uuid);
    valuelist.push(data.voucher_out_id);
    valuelist.push(data.stockout_date);
    valuelist.push(data.stockout_time);
    valuelist.push(data.receive_uuid);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.voucher_out_id = req.body["voucher_out_id"];
    model.stockout_date = req.body["stockout_date"];
    model.stockout_time = req.body["stockout_time"];
    model.receive_uuid = req.body["receive_uuid"];
    model.sender_uuid = req.body["sender_uuid"];
    model.ref_out = req.body["ref_out"];
...
let getLabel = (model) => {
  try {
    return model["voucher_out_id"] + " - " + model["stockout_date"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.stockout_uuid = req.body["stockout_uuid"];
    model.voucher_out_id = req.body["voucher_out_id"];
    model.stockout_date = req.body["stockout_date"];
    model.stockout_time = req.body["stockout_time"];
    model.receive_uuid = req.body["receive_uuid"];
