...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("assessment_trans_name");
    keyList.push("assessment_trans_code");
    keyList.push("employee_uuid");
    keyList.push("assessment_trans_type_uuid");
    keyList.push("assessment_trans_date");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("assessment_trans_uuid");
    keyList.push("assessment_trans_name");
    keyList.push("assessment_trans_code");
    keyList.push("employee_uuid");
    keyList.push("assessment_trans_type_uuid");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO assessment_trans_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE assessment_trans_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE assessment_trans_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_assessment_trans_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE assessment_trans_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE assessment_trans_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_assessment_trans_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_assessment_trans_table_item WHERE assessment_trans_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.assessment_trans_name);
    valuelist.push(data.assessment_trans_code);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.assessment_trans_type_uuid);
    valuelist.push(data.assessment_trans_date);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.assessment_trans_uuid);
    valuelist.push(data.assessment_trans_name);
    valuelist.push(data.assessment_trans_code);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.assessment_trans_type_uuid);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.assessment_trans_name = req.body["assessment_trans_name"];
    model.assessment_trans_code = req.body["assessment_trans_code"];
    model.employee_uuid = req.body["employee_uuid"];
    model.assessment_trans_type_uuid = req.body["assessment_trans_type_uuid"];
    model.assessment_trans_date = mTools.toDateCurrectFormat(
      req.body["assessment_trans_date"]
    );
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["assessment_trans_uuid"];
...
let getLabel = (model) => {
  try {
    return (
      model["assessment_trans_code"] + " - " + model["assessment_trans_name"]
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.assessment_trans_uuid = req.body["assessment_trans_uuid"];
    model.assessment_trans_name = req.body["assessment_trans_name"];
    model.assessment_trans_code = req.body["assessment_trans_code"];
    model.employee_uuid = req.body["employee_uuid"];
    model.assessment_trans_type_uuid = req.body["assessment_trans_type_uuid"];
...
let getSqlGetLastAssessmentTransCode = (req) => {
  return (
    "SELECT * FROM vw_assessment_trans_table_item " +
    mTools.getConditionCompanyAndBranch(req) +
    " ORDER BY assessment_trans_code DESC LIMIT 1"
  );
};
module.exports = {
  toDropdown,
  ...
  getSqlDropdown,
  getRoWAuth,
  getSqlGetLastAssessmentTransCode,
};
