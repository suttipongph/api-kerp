...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("document_status_code");
    keyList.push("document_status_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO document_status_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE document_status_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE document_status_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_document_status_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE document_status_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE document_status_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_document_status_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_document_status_table_item WHERE document_status_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.document_status_code);
    valuelist.push(data.document_status_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.document_status_code = req.body["document_status_code"];
    model.document_status_name = req.body["document_status_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["document_status_uuid"];
...
let getLabel = (model) => {
  try {
    return (
      model["document_status_code"] + " - " + model["document_status_name"]
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getDocumentStatusByReftype = (ref_type, statusId) => {
  try {
    return (
      "SELECT * FROM vw_document_status_table_dropdown WHERE ref_type = " +
      ref_type +
      " " +
      "AND document_status_code = " +
      "'" +
      statusId +
      "'"
    );
