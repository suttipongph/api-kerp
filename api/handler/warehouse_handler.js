...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("warehouse_code");
    keyList.push("warehouse_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE warehouse_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE warehouse_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_warehouse_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE warehouse_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE warehouse_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_warehouse_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_warehouse_table_item WHERE warehouse_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.warehouse_code);
    valuelist.push(data.warehouse_name);
    valuelist.push(data.location);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.warehouse_code = req.body["warehouse_code"];
    model.warehouse_name = req.body["warehouse_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["warehouse_uuid"];
...
let getLabel = (model) => {
  try {
    return model["warehouse_code"] + " - " + model["warehouse_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
