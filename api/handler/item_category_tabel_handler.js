...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("item_category_name");
    keyList.push("item_category_code");
    keyList.push("remark");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("item_category_uuid");
    keyList.push("item_category_name");
    keyList.push("item_category_code");
    keyList.push("remark");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE item_category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE item_category_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_item_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE item_category_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE item_category_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_item_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_item_category_table_item WHERE item_category_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.item_category_name);
    valuelist.push(data.item_category_code);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.item_category_uuid);
    valuelist.push(data.item_category_name);
    valuelist.push(data.item_category_code);
    valuelist.push(data.remark);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.item_category_name = req.body["item_category_name"];
    model.item_category_code = req.body["item_category_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["item_category_uuid"];
...
let getLabel = (model) => {
  try {
    return model["item_category_code"] + " - " + model["item_category_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.item_category_uuid = req.body["item_category_uuid"];
    model.item_category_name = req.body["item_category_name"];
    model.item_category_code = req.body["item_category_code"];
    model.remark = req.body["remark"];
