...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("welfare_code");
    keyList.push("welfare_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE welfare_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE welfare_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_welfare_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE welfare_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE welfare_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_welfare_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_welfare_table_item WHERE welfare_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.welfare_code);
    valuelist.push(data.welfare_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.welfare_code = req.body["welfare_code"];
    model.welfare_name = req.body["welfare_name"];
...
let getLabel = (model) => {
  try {
    return model["welfare_code"] + " - " + model["welfare_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
