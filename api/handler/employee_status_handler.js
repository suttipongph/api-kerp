...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("employee_status_name");
    keyList.push("employee_status_code");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("employee_status_uuid");
    keyList.push("employee_status_name");
    keyList.push("employee_status_code");
    keyList.push("description");
    keyList.push("remark");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO employee_status_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE employee_status_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE employee_status_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_employee_status_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE employee_status_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE employee_status_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_employee_status_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_employee_status_table_item WHERE employee_status_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.employee_status_name);
    valuelist.push(data.employee_status_code);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.employee_status_uuid);
    valuelist.push(data.employee_status_name);
    valuelist.push(data.employee_status_code);
    valuelist.push(data.description);
    valuelist.push(data.remark);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.employee_status_name = req.body["employee_status_name"];
    model.employee_status_code = req.body["employee_status_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["employee_status_uuid"];
...
let getLabel = (model) => {
  try {
    return model["employee_status_code"] + " - " + model["employee_status_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.employee_status_uuid = req.body["employee_status_uuid"];
    model.employee_status_name = req.body["employee_status_name"];
    model.employee_status_code = req.body["employee_status_code"];
    model.description = req.body["description"];
    model.remark = req.body["remark"];
