...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("job_description_code");
    keyList.push("job_description_name");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("job_description_uuid");
    keyList.push("job_description_code");
    keyList.push("job_description_name");
    keyList.push("description");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE job_description_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE job_description_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_job_description_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE job_description_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE job_description_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_job_description_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_job_description_table_item WHERE job_description_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.job_description_code);
    valuelist.push(data.job_description_name);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.job_description_uuid);
    valuelist.push(data.job_description_code);
    valuelist.push(data.job_description_name);
    valuelist.push(data.description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.job_description_code = req.body["job_description_code"];
    model.job_description_name = req.body["job_description_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["job_description_uuid"];
...
let getLabel = (model) => {
  try {
    return model["job_description_code"] + " - " + model["job_description_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.job_description_uuid = req.body["job_description_uuid"];
    model.job_description_code = req.body["job_description_code"];
    model.job_description_name = req.body["job_description_name"];
    model.description = req.body["description"];
