...
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_access_rights_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_access_rights_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.access_rights_code = req.body["access_rights_code"];
    model.access_rights_name = req.body["access_rights_name"];
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.access_rights_uuid = req.body["access_rights_uuid"];
    model.access_rights_code = req.body["access_rights_code"];
    model.access_rights_name = req.body["access_rights_name"];
    model.role_uuid = req.body["role_uuid"];
    model.remark = req.body["remark"];
