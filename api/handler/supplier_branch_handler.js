...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("supplier_branch_code");
    keyList.push("supplier_branch_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO supplier_branch_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE supplier_branch_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE supplier_branch_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_supplier_branch_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE supplier_branch_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE supplier_branch_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_supplier_branch_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_supplier_branch_table_item WHERE supplier_branch_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.supplier_branch_code);
    valuelist.push(data.supplier_branch_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.supplier_branch_code = req.body["supplier_branch_code"];
    model.supplier_branch_name = req.body["supplier_branch_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["supplier_branch_uuid"];
...
let getLabel = (model) => {
  try {
    return model["supplier_branch_code"] + " - " + model["supplier_branch_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
