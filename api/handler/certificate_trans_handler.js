...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("certificate_trans_name");
    keyList.push("certificate_trans_code");
    keyList.push("certificate_type_uuid");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("certificate_trans_uuid");
    keyList.push("certificate_trans_name");
    keyList.push("certificate_trans_code");
    keyList.push("certificate_type_uuid");
    keyList.push("description");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO certificate_trans_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE certificate_trans_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE certificate_trans_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_certificate_trans_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE certificate_trans_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE certificate_trans_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_certificate_trans_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_certificate_trans_table_item WHERE certificate_trans_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.certificate_trans_name);
    valuelist.push(data.certificate_trans_code);
    valuelist.push(data.certificate_type_uuid);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.certificate_trans_uuid);
    valuelist.push(data.certificate_trans_name);
    valuelist.push(data.certificate_trans_code);
    valuelist.push(data.certificate_type_uuid);
    valuelist.push(data.description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.certificate_trans_name = req.body["certificate_trans_name"];
    model.certificate_trans_code = req.body["certificate_trans_code"];
    model.certificate_type_uuid = req.body["certificate_type_uuid"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["certificate_trans_uuid"];
...
let getLabel = (model) => {
  try {
    return model["certificate_trans_name"] + " - " + model["certificate_trans_code"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.certificate_trans_uuid = req.body["certificate_trans_uuid"];
    model.certificate_trans_name = req.body["certificate_trans_name"];
    model.certificate_trans_code = req.body["certificate_trans_code"];
    model.certificate_type_uuid = req.body["certificate_type_uuid"];
    model.description = req.body["description"];
...
let getSqlGetLastCertificateTransCode = (req) => {
  return (
    "SELECT * FROM vw_certificate_trans_table_item " +
    mTools.getConditionCompanyAndBranch(req) +
    " ORDER BY certificate_trans_code DESC LIMIT 1"
  );
};
module.exports = {
  toDropdown,
  ...
  getSqlDropdown,
  getRoWAuth,
  getSqlGetLastCertificateTransCode,
};
