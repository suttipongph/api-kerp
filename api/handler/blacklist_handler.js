...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("blacklist_code");
    keyList.push("blacklist_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE blacklist_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE blacklist_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_blacklist_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE blacklist_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE blacklist_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_blacklist_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_blacklist_table_item WHERE blacklist_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.blacklist_code);
    valuelist.push(data.blacklist_name);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.blacklist_uuid);
    valuelist.push(data.blacklist_code);
    valuelist.push(data.blacklist_name);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.blacklist_code = req.body["blacklist_code"];
    model.blacklist_name = req.body["blacklist_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["blacklist_uuid"];
...
let getLabel = (model) => {
  try {
    return model["blacklist_code"] + " - " + model["blacklist_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.blacklist_uuid = req.body["blacklist_uuid"];
    model.blacklist_code = req.body["blacklist_code"];
    model.blacklist_name = req.body["blacklist_name"];
    model.employee_uuid = req.body["employee_uuid"];
    model.description = req.body["description"];
