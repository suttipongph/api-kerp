...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("company_category_code");
    keyList.push("company_category_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO company_category_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE company_category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE company_category_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_company_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE company_category_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE company_category_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_company_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_company_category_table_item WHERE company_category_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.company_category_code);
    valuelist.push(data.company_category_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.company_category_code = req.body["company_category_code"];
    model.company_category_name = req.body["company_category_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["company_category_uuid"];
...
let getLabel = (model) => {
  try {
    return model["company_category_code"] + " - " + model["company_category_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
