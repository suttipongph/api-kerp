...
let getSqlDelete = (model) => {
  try {
    return "UPDATE category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE category_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      ...
      index +
      " ) " +
      "WHERE category_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_category_table_item WHERE category_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.category_code);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.category_uuid);
    valuelist.push(data.category_code);
    valuelist.push(data.note);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.category_code = req.body["category_code"];
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.category_uuid = req.body["category_uuid"];
    model.category_code = req.body["category_code"];
    model.note = req.body["note"];
