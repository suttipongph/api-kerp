...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("legal_punishment_code");
    keyList.push("legal_punishment_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO legal_punishment_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE legal_punishment_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE legal_punishment_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_legal_punishment_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE legal_punishment_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE legal_punishment_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_legal_punishment_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_legal_punishment_table_item WHERE legal_punishment_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.legal_punishment_code);
    valuelist.push(data.legal_punishment_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.legal_punishment_code = req.body["legal_punishment_code"];
    model.legal_punishment_name = req.body["legal_punishment_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["legal_punishment_uuid"];
...
let getLabel = (model) => {
  try {
    return model["legal_punishment_code"] + " - " + model["legal_punishment_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
