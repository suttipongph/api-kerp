...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("department_code");
    keyList.push("department_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE department_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE department_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_department_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      ...
      index +
      " ) " +
      "WHERE department_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_department_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_department_table_item WHERE department_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.department_code);
    valuelist.push(data.department_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.department_code = req.body["department_code"];
    model.department_name = req.body["department_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["department_uuid"];
...
let getLabel = (model) => {
  try {
    return model["department_code"] + " - " + model["department_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
