...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("certificate_type_name");
    keyList.push("certificate_type_code");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO certificate_type_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE certificate_type_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE certificate_type_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_certificate_type_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE certificate_type_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE certificate_type_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_certificate_type_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_certificate_type_table_item WHERE certificate_type_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.certificate_type_name);
    valuelist.push(data.certificate_type_code);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.certificate_type_name = req.body["certificate_type_name"];
    model.certificate_type_code = req.body["certificate_type_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["certificate_type_uuid"];
...
let getLabel = (model) => {
  try {
    return model["certificate_type_name"] + " - " + model["certificate_type_code"];
  } catch (e) {
    throw new Error(e.message);
  }
};
