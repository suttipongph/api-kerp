...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("legal_punishment_detail_code");
    keyList.push("legal_punishment_detail_name");
    keyList.push("legal_punishment_uuid");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("legal_punishment_detail_uuid");
    keyList.push("legal_punishment_detail_code");
    keyList.push("legal_punishment_detail_name");
    keyList.push("legal_punishment_uuid");
    keyList.push("description");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO legal_punishment_detail_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE legal_punishment_detail_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE legal_punishment_detail_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_legal_punishment_detail_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE legal_punishment_detail_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE legal_punishment_detail_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_legal_punishment_detail_table_list " +
      mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_legal_punishment_detail_table_item WHERE legal_punishment_detail_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.legal_punishment_detail_code);
    valuelist.push(data.legal_punishment_detail_name);
    valuelist.push(data.legal_punishment_uuid);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.legal_punishment_detail_uuid);
    valuelist.push(data.legal_punishment_detail_code);
    valuelist.push(data.legal_punishment_detail_name);
    valuelist.push(data.legal_punishment_uuid);
    valuelist.push(data.description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.legal_punishment_detail_code =
      req.body["legal_punishment_detail_code"];
    model.legal_punishment_detail_name =
      req.body["legal_punishment_detail_name"];
    model.legal_punishment_uuid = req.body["legal_punishment_uuid"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["legal_punishment_detail_uuid"];
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.legal_punishment_detail_uuid =
      req.body["legal_punishment_detail_uuid"];
    model.legal_punishment_detail_code =
      req.body["legal_punishment_detail_code"];
    model.legal_punishment_detail_name =
      req.body["legal_punishment_detail_name"];
