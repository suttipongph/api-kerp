...
let getKeysCreate = (req) => {
  try {
    ...
    keyList.push("registration_date");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE company_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE company_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_company_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      ...
      index +
      " ) " +
      "WHERE company_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_company_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_company_table_item WHERE company_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    valuelist.push(mTools.toDateCurrectFormat(data.registration_date));
...
let toCreateModel = (req) => {
  try {
    ...
    model.registration_date = req.body["registration_date"];
...
let getLabel = (model) => {
  try {
    return model["address"] + " - " + model["company_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
