...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("user_password");
    keyList.push("user_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO user_table (" + keys + ") values (" + index + ") RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE user_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE user_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return "SELECT * FROM vw_user_table_dropdown " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_user_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_user_table_item WHERE user_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.user_password);
    valuelist.push(data.user_name);
    ...
    valuelist.push(data.languageid);
    return valuelist;
  } catch (e) {
    throw new Error(e.message);
  }
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.user_password = bcrypt.hashSync(req.body["user_password"], 10);
    model.user_name = req.body["user_name"];
    model.employee_uuid = req.body["employee_uuid"];
    model.subscribe_uuid = req.body["subscribe_uuid"];
    model.role_uuid = req.body["role_uuid"];
    ...
    model.languageid = req.body["languageid"];
    return model;
  } catch (e) {
    throw new Error(e.message);
  }
...
let getLabel = (model) => {
  try {
    return model["user_name"] + " - " + model["employee_uuid"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.user_uuid = req.body["user_uuid"];
    //  model.user_password = req.body["user_password"];
    model.user_name = req.body["user_name"];
    model.employee_uuid = req.body["employee_uuid"];
    model.subscribe_uuid = req.body["subscribe_uuid"];
    model.role_uuid = req.body["role_uuid"];
