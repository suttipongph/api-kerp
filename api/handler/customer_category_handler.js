...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("customer_category_name");
    keyList.push("description");
    keyList.push("customer_category_code");
    keyList.push("remark");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO customer_category_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE customer_category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE customer_category_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_customer_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE customer_category_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE customer_category_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_customer_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_customer_category_table_item WHERE customer_category_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.customer_category_name);
    valuelist.push(data.description);
    valuelist.push(data.customer_category_code);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.customer_category_name = req.body["customer_category_name"];
    model.description = req.body["description"];
    model.customer_category_code = req.body["customer_category_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["customer_category_uuid"];
...
let getLabel = (model) => {
  try {
    return model["customer_category_code"] + " - " + model["customer_category_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getSqlGetLastCustomerCategoryCode = (req) => {
  return (
    "SELECT * FROM vw_customer_category_table_item " +
    mTools.getConditionCompanyAndBranch(req) +
    " ORDER BY customer_category_name DESC LIMIT 1"
  );
};
module.exports = {
  toDropdown,
  ...
  getSqlDropdown,
  getRoWAuth,
  getSqlGetLastCustomerCategoryCode
};
