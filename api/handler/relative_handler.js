...
let getSqlDelete = (model) => {
  try {
    return "UPDATE relative_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE relative_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_relative_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE relative_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE relative_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_relative_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_relative_table_item WHERE relative_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.relative_uuid);
    valuelist.push(data.name);
    valuelist.push(data.distance);
    valuelist.push(data.address);
    valuelist.push(data.implicate);
...
let toCreateModel = (req) => {
  try {
    ...
    model.distance = req.body["distance"];
...
let getLabel = (model) => {
  try {
    return model["name"] + " - " + model["implicate"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.relative_uuid = req.body["relative_uuid"];
    model.name = req.body["name"];
    model.distance = req.body["distance"];
    model.address = req.body["address"];
    model.implicate = req.body["implicate"];
