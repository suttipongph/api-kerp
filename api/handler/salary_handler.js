...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("salary_type");
    keyList.push("salary_value");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE salary_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE salary_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_salary_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE salary_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE salary_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_salary_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_salary_table_item WHERE salary_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.salary_type);
    valuelist.push(data.salary_value);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.salary_type = req.body["salary_type"];
    model.salary_value = req.body["salary_value"];
...
let getLabel = (model) => {
  try {
    return model["salary_type"] + " - " + model["salary_value"];
  } catch (e) {
    throw new Error(e.message);
  }
};
