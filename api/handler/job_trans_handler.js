...
let getKeysCreate = (req) => {
  try {
    ...
    keyList.push("date");
    keyList.push("jobtime_start");
    keyList.push("jobtime_end");
    keyList.push("remark");
    keyList.push("line");
    keyList.push("work_station");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE job_trans_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE job_trans_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_job_trans_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE job_trans_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE job_trans_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_job_trans_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_job_trans_table_item WHERE job_trans_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    valuelist.push(data.jobtime_start);
    valuelist.push(data.jobtime_end);
    valuelist.push(data.remark);
    valuelist.push(data.line);
    valuelist.push(data.work_station);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.job_trans_uuid);
    valuelist.push(data.job_uuid);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.date);
    valuelist.push(data.jobtime_start);
...
let toCreateModel = (req) => {
  try {
    ...
    model.jobtime_start = req.body["jobtime_start"];
    model.jobtime_end = req.body["jobtime_end"];
    model.remark = req.body["remark"];
    model.line = req.body["line"];
    model.work_station = req.body["work_station"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["job_trans_uuid"];
...
let getLabel = (model) => {
  try {
    return model["employee_uuid"] + " - " + model["job_uuid"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.job_trans_uuid = req.body["job_trans_uuid"];
    model.job_uuid = req.body["job_uuid"];
    model.employee_uuid = req.body["employee_uuid"];
    model.date = req.body["date"];
    model.jobtime_start = req.body["jobtime_start"];
