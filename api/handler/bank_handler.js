...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("bank_code");
    keyList.push("bank_name");
    keyList.push("bank_branch");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE bank_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE bank_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_bank_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE bank_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE bank_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_bank_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_bank_table_item WHERE bank_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.bank_code);
    valuelist.push(data.bank_name);
    valuelist.push(data.bank_branch);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.bank_code = req.body["bank_code"];
    model.bank_name = req.body["bank_name"];
    model.bank_branch = req.body["bank_branch"];
...
let getLabel = (model) => {
  try {
    return model["bank_name"] + " - " + model["bank_branch"];
  } catch (e) {
    throw new Error(e.message);
  }
};
