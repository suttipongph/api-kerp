...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("reject_trans_uuid");
    keyList.push("description");
    keyList.push("remark");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO reject_trans_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE reject_trans_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE reject_trans_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_reject_trans_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE reject_trans_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE reject_trans_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_reject_trans_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_reject_trans_table_item WHERE reject_trans_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.reject_trans_uuid);
    valuelist.push(data.description);
    valuelist.push(data.remark);
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["reject_trans_uuid"];
...
let getLabel = (model) => {
  try {
    return model["description"] + " - " + model[""];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.reject_trans_uuid = req.body["reject_trans_uuid"];
    model.description = req.body["description"];
    model.remark = req.body["remark"];
