...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("employee_catagory_name");
    keyList.push("description");
    keyList.push("employee_cetagory_code");
    keyList.push("remark");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("employee_catagory_uuid");
    keyList.push("employee_catagory_name");
    keyList.push("description");
    keyList.push("employee_cetagory_code");
    keyList.push("remark");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO employee_category_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE employee_category_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE employee_catagory_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_employee_category_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE employee_category_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE employee_catagory_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_employee_category_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_employee_category_table_item WHERE employee_catagory_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.employee_catagory_name);
    valuelist.push(data.description);
    valuelist.push(data.employee_cetagory_code);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.employee_catagory_uuid);
    valuelist.push(data.employee_catagory_name);
    valuelist.push(data.description);
    valuelist.push(data.employee_cetagory_code);
    valuelist.push(data.remark);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.employee_catagory_name = req.body["employee_catagory_name"];
    model.description = req.body["description"];
    model.employee_cetagory_code = req.body["employee_cetagory_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["employee_catagory_uuid"];
...
let getLabel = (model) => {
  try {
    return model["employee_cetagory_code"] + " - " + model["employee_catagory_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.employee_catagory_uuid = req.body["employee_catagory_uuid"];
    model.employee_catagory_name = req.body["employee_catagory_name"];
    model.description = req.body["description"];
    model.employee_cetagory_code = req.body["employee_cetagory_code"];
    model.remark = req.body["remark"];
