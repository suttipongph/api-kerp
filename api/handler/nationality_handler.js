...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("nationality_name");
    keyList.push("nationality_code");
    keyList.push("remark");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE nationality_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE nationality_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_nationality_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE nationality_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE nationality_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_nationality_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_nationality_table_item WHERE nationality_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.nationality_name);
    valuelist.push(data.nationality_code);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.nationality_name = req.body["nationality_name"];
    model.nationality_code = req.body["nationality_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["nationality_uuid"];
...
let getLabel = (model) => {
  try {
    return model["nationality_code"] + " - " + model["nationality_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
