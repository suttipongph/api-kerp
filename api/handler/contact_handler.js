...
let getSqlDelete = (model) => {
  try {
    return "UPDATE contact_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE contact_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_contact_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE contact_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE contact_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_contact_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_contact_table_item WHERE contact_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getLabel = (model) => {
  try {
    return model["phone_number"] + " - " + model["line"];
  } catch (e) {
    throw new Error(e.message);
  }
};
