...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("training_trans_detail_code");
    keyList.push("training_trans_detail_name");
    keyList.push("employee_uuid");
    keyList.push("department_uuid");
    keyList.push("position_uuid");
    keyList.push("description");
    keyList.push("training_trans_uuid");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("training_trans_detail_uuid");
    keyList.push("training_trans_detail_code");
    keyList.push("training_trans_detail_name");
    keyList.push("employee_uuid");
    keyList.push("department_uuid");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO training_trans_detail_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE training_trans_detail_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE training_trans_detail_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_training_trans_detail_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE training_trans_detail_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE training_trans_detail_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_training_trans_detail_table_list " +
      mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_training_trans_detail_table_item WHERE training_trans_detail_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.training_trans_detail_code);
    valuelist.push(data.training_trans_detail_name);
    ...
    valuelist.push(data.training_trans_uuid);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.training_trans_detail_uuid);
    valuelist.push(data.training_trans_detail_code);
    valuelist.push(data.training_trans_detail_name);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.department_uuid);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.training_trans_detail_code = req.body["training_trans_detail_code"];
    model.training_trans_detail_name = req.body["training_trans_detail_name"];
    ...
    model.training_trans_uuid = req.body["training_trans_uuid"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["training_trans_detail_uuid"];
...
let getLabel = (model) => {
  try {
    return (
      model["training_trans_detail_name"] + " - " + model["employee_value"]
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.training_trans_detail_uuid = req.body["training_trans_detail_uuid"];
    model.training_trans_detail_code = req.body["training_trans_detail_code"];
    model.training_trans_detail_name = req.body["training_trans_detail_name"];
    model.employee_uuid = req.body["employee_uuid"];
    model.department_uuid = req.body["department_uuid"];
...
let getSqlGetLastTrainingTransDetailCode = (req) => {
  return (
    "SELECT * FROM vw_training_trans_detail_table_item " +
    mTools.getConditionCompanyAndBranch(req) +
    " ORDER BY training_trans_detail_code DESC LIMIT 1"
  );
};
module.exports = {
  toDropdown,
  ...
  getSqlDropdown,
  getRoWAuth,
  getSqlGetLastTrainingTransDetailCode
};
