...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("attachment_file");
    keyList.push("file_description");
    ...
    keyList.push("content_type");
    keyList.push("file_name");
    keyList.push("file_path");
    return keyList;
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE attachment_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE attachment_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_attachment_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE attachment_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE attachment_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  console.log("getSqlList", req.body);
  try {
    return "SELECT * FROM vw_attachment_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_attachment_table_item WHERE attachment_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  console.log("getValuesCreate ==>", model.body);
  try {
    var data = toCreateModel(model);
    let valuelist = [];
    valuelist.push(data.attachment_file);
    valuelist.push(data.file_description);
    ...
    valuelist.push(data.content_type);
    valuelist.push(data.file_name);
    valuelist.push(data.file_path);
    return valuelist;
  } catch (e) {
    throw new Error(e.message);
  }
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.attachment_uuid);
    valuelist.push(data.attachment_file);
    valuelist.push(data.file_description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.attachment_file = req.body["attachment_file"];
    model.file_description = req.body["file_description"];
    ...
    model.content_type = req.body["content_type"];
    model.file_name = req.body["file_name"];
    model.file_path = req.body["file_path"];
    return model;
  } catch (e) {
    throw new Error(e.message);
  }
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["attachment_uuid"];
...
let getLabel = (model) => {
  try {
    return model["attachment_file"] + " - " + model["file_description"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.attachment_uuid = req.body["attachment_uuid"];
    model.attachment_file = req.body["attachment_file"];
    model.file_description = req.body["file_description"];
