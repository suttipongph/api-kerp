...
let getSqlDelete = (model) => {
  try {
    return "UPDATE status_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE status_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return "SELECT * FROM vw_status_table_dropdown " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_status_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_status_table_item WHERE status_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.status_code);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.status_code = req.body["status_code"];
...
let getLabel = (model) => {
  try {
    return model["status_code"] + " - " + model["description"];
  } catch (e) {
    throw new Error(e.message);
  }
};
