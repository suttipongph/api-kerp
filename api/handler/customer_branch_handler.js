...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("customer_branch_code");
    keyList.push("customer_branch_name");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO customer_branch_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE customer_branch_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE customer_branch_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_customer_branch_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE customer_branch_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE customer_branch_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_customer_branch_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_customer_branch_table_item WHERE customer_branch_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.customer_branch_code);
    valuelist.push(data.customer_branch_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.customer_branch_code = req.body["customer_branch_code"];
    model.customer_branch_name = req.body["customer_branch_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["customer_branch_uuid"];
...
let getLabel = (model) => {
  try {
    return (
      model["customer_branch_code"] + " - " + model["customer_branch_name"]
    );
  } catch (e) {
    throw new Error(e.message);
  }
