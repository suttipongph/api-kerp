...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("part_code");
    keyList.push("part_name");
    keyList.push("amount");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE part_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE part_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_part_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE part_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE part_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_part_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_part_table_item WHERE part_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.part_code);
    valuelist.push(data.part_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.part_code = req.body["part_code"];
    model.part_name = req.body["part_name"];
...
let getLabel = (model) => {
  try {
    return model["part_code"] + " - " + model["part_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
