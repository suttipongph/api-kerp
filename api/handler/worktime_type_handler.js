...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("worktime_type_name");
    keyList.push("compensation");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO worktime_type_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE worktime_type_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE worktime_type_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_worktime_type_table_dropdown " +
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE worktime_type_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE worktime_type_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return (
      "SELECT * FROM vw_worktime_type_table_list " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_worktime_type_table_item WHERE worktime_type_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.worktime_type_name);
    valuelist.push(data.compensation);
    ...
    valuelist.push(data.deduction_rate);
    return valuelist;
  } catch (e) {
    throw new Error(e.message);
  }
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.worktime_type_name = req.body["worktime_type_name"];
    model.compensation = req.body["compensation"];
    ...
    model.deduction_rate = req.body["deduction_rate"];
    return model;
  } catch (e) {
    throw new Error(e.message);
  }
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["worktime_type_uuid"];
...
let getLabel = (model) => {
  try {
    return model["worktime_type_name"] + " - " + model["compensation"];
  } catch (e) {
    throw new Error(e.message);
  }
};
