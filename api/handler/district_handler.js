...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("district_code");
    keyList.push("district_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE district_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE district_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_district_table_dropdown " + mTools.getConditionOnly(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE district_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE district_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_district_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_district_table_item WHERE district_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.district_code);
    valuelist.push(data.district_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.district_code = req.body["district_code"];
    model.district_name = req.body["district_name"];
...
let getLabel = (model) => {
  try {
    return model["district_code"] + " - " + model["district_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
