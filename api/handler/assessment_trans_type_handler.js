...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("assessment_trans_type_name");
    keyList.push("assessment_trans_type_code");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO assessment_trans_type_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE assessment_trans_type_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE assessment_trans_type_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_assessment_trans_type_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE assessment_trans_type_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE assessment_trans_type_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_assessment_trans_type_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_assessment_trans_type_table_item WHERE assessment_trans_type_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.assessment_trans_type_name);
    valuelist.push(data.assessment_trans_type_code);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.assessment_trans_type_name = req.body["assessment_trans_type_name"];
    model.assessment_trans_type_code = req.body["assessment_trans_type_code"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["assessment_trans_type_uuid"];
...
let getLabel = (model) => {
  try {
    return model["assessment_trans_type_name"] + " - " + model["assessment_trans_type_code"];
  } catch (e) {
    throw new Error(e.message);
  }
};
