...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("customer_type_code");
    keyList.push("customer_type_name");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("customer_type_uuid");
    keyList.push("customer_type_code");
    keyList.push("customer_type_name");
    keyList.push("description");
    keyList.push("remark");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE customer_type_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE customer_type_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_customer_type_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE customer_type_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE customer_type_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_customer_type_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_customer_type_table_item WHERE customer_type_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.customer_type_code);
    valuelist.push(data.customer_type_name);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.customer_type_uuid);
    valuelist.push(data.customer_type_code);
    valuelist.push(data.customer_type_name);
    valuelist.push(data.description);
    valuelist.push(data.remark);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.customer_type_code = req.body["customer_type_code"];
    model.customer_type_name = req.body["customer_type_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["customer_type_uuid"];
...
let getLabel = (model) => {
  try {
    return model["customer_type_code"] + " - " + model["customer_type_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.customer_type_uuid = req.body["customer_type_uuid"];
    model.customer_type_code = req.body["customer_type_code"];
    model.customer_type_name = req.body["customer_type_name"];
    model.description = req.body["description"];
    model.remark = req.body["remark"];
