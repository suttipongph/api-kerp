...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("job_name");
    keyList.push("co_uuid");
    keyList.push("maker_uuid");
    keyList.push("date");
    keyList.push("production_uuid");
    keyList.push("remark");
    keyList.push("co_id");
    keyList.push("maker_id");
    keyList.push("production_id");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE job_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE job_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_job_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_job_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_job_table_item WHERE job_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.job_name);
    valuelist.push(data.co_uuid);
    valuelist.push(data.maker_uuid);
    valuelist.push(mTools.toDateCurrectFormat(data.date));
    valuelist.push(data.production_uuid);
    valuelist.push(data.remark);
    valuelist.push(data.co_id);
    valuelist.push(data.maker_id);
    valuelist.push(data.production_id);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.job_name = req.body["job_name"];
    model.co_uuid = req.body["co_uuid"];
    model.maker_uuid = req.body["maker_uuid"];
    model.date = req.body["date"];
    model.production_uuid = req.body["production_uuid"];
    model.remark = req.body["remark"];
    model.co_id = req.body["co_id"];
    model.maker_id = req.body["maker_id"];
    model.production_id = req.body["production_id"];
...
let getLabel = (model) => {
  try {
    return model["job_name"] + " - " + model["co_id"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.job_uuid = req.body["job_uuid"];
    model.job_name = req.body["job_name"];
    model.co_uuid = req.body["co_uuid"];
    model.maker_uuid = req.body["maker_uuid"];
    model.date = req.body["date"];
