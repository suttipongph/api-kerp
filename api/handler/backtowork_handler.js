...
let getSqlDelete = (model) => {
  try {
    return "UPDATE backtowork_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE backtowork_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_backtowork_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE backtowork_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE backtowork_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_backtowork_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_backtowork_table_item WHERE backtowork_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    valuelist.push(data.backtowork_date);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.backtowork_uuid);
    valuelist.push(data.remark);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.position_uuid);
    valuelist.push(data.department_uuid);
...
let toCreateModel = (req) => {
  try {
    ...
    model.backtowork_date = req.body["backtowork_date"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["backtowork_uuid"];
...
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.backtowork_uuid = req.body["backtowork_uuid"];
    model.remark = req.body["remark"];
    model.employee_uuid = req.body["employee_uuid"];
    model.position_uuid = req.body["position_uuid"];
    model.department_uuid = req.body["department_uuid"];
