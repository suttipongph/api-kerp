...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("position_code");
    keyList.push("position_name");
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE position_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE position_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_position_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      ...
      index +
      " ) " +
      "WHERE position_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_position_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_position_table_item WHERE position_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.position_code);
    valuelist.push(data.position_name);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.position_code = req.body["position_code"];
...
let getLabel = (model) => {
  try {
    return model["position_code"] + " - " + model["position_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
...
let getFielChangeDropdown = (req, depId) => {
  try {
    return (
      "SELECT * FROM vw_position_table_dropdown " +
      mTools.getConditionFieldChange(req) +
      "AND department_uuid = '" +
      depId +
      "'"
    );
