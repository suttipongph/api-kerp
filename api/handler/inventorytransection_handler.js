...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("invent_trans_no");
    keyList.push("part_uuid");
    keyList.push("part_qty");
    keyList.push("balance");
    keyList.push("last_balance");
    keyList.push("last_price");
    keyList.push("move_type");
    keyList.push("remark");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("inventorytransection_uuid");
    keyList.push("invent_trans_no");
    keyList.push("part_uuid");
    keyList.push("part_qty");
    keyList.push("balance");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO inventorytransection_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE inventorytransection_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE inventorytransection_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_inventorytransection_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE inventorytransection_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE inventorytransection_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_inventorytransection_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_inventorytransection_table_item WHERE inventorytransection_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.invent_trans_no);
    valuelist.push(data.part_uuid);
    valuelist.push(data.part_qty);
    valuelist.push(data.balance);
    valuelist.push(data.last_balance);
    valuelist.push(data.last_price);
    valuelist.push(data.move_type);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.inventorytransection_uuid);
    valuelist.push(data.invent_trans_no);
    valuelist.push(data.part_uuid);
    valuelist.push(data.part_qty);
    valuelist.push(data.balance);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.invent_trans_no = req.body["invent_trans_no"];
    model.part_uuid = req.body["part_uuid"];
    model.part_qty = req.body["part_qty"];
    model.balance = req.body["balance"];
    model.last_balance = req.body["last_balance"];
    model.last_price = req.body["last_price"];
    model.move_type = req.body["move_type"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["inventorytransection_uuid"];
...
let getLabel = (model) => {
  try {
    return model["invent_trans_no"] + " - " + model["part_uuid"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.inventorytransection_uuid = req.body["inventorytransection_uuid"];
    model.invent_trans_no = req.body["invent_trans_no"];
    model.part_uuid = req.body["part_uuid"];
    model.part_qty = req.body["part_qty"];
    model.balance = req.body["balance"];
