...
let getKeysCreate = (req) => {
  try {
    let keyList = [];
    keyList.push("welfare_detail_code");
    keyList.push("welfare_detail_name");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("welfare_detail_uuid");
    keyList.push("welfare_detail_code");
    keyList.push("welfare_detail_name");
    keyList.push("welfare_uuid");
    keyList.push("description");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO welfare_detail_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE welfare_detail_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE welfare_detail_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_welfare_detail_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE welfare_detail_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE welfare_detail_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_welfare_detail_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_welfare_detail_table_item WHERE welfare_detail_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.welfare_detail_code);
    valuelist.push(data.welfare_detail_name);
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.welfare_detail_uuid);
    valuelist.push(data.welfare_detail_code);
    valuelist.push(data.welfare_detail_name);
    valuelist.push(data.welfare_uuid);
    valuelist.push(data.description);
...
let toCreateModel = (req) => {
  try {
    ...
    var model = {};
    model.welfare_detail_code = req.body["welfare_detail_code"];
    model.welfare_detail_name = req.body["welfare_detail_name"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["welfare_detail_uuid"];
...
let getLabel = (model) => {
  try {
    return model["welfare_detail_code"] + " - " + model["welfare_detail_name"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.welfare_detail_uuid = req.body["welfare_detail_uuid"];
    model.welfare_detail_code = req.body["welfare_detail_code"];
    model.welfare_detail_name = req.body["welfare_detail_name"];
    model.welfare_uuid = req.body["welfare_uuid"];
    model.description = req.body["description"];
