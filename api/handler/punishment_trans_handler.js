...
let getKeysCreate = (req) => {
  try {
    ...
    keyList.push("punishment_type_uuid");
    keyList.push("total_amount");
    keyList.push("period");
    keyList.push("amount_period");
    keyList.push("start_date");
    keyList.push("end_date");
...
let getKeysEdit = (req) => {
  try {
    let keyList = [];
    keyList.push("punishment_trans_uuid");
    keyList.push("employee_uuid");
    keyList.push("punishment_type_uuid");
    keyList.push("total_amount");
    keyList.push("period");
...
let getSqlCreate = (req) => {
  try {
    ...
    return (
      "INSERT INTO punishment_trans_table (" +
      keys +
      ") values (" +
      index +
      ") RETURNING *"
...
let getSqlDelete = (model) => {
  try {
    return "UPDATE punishment_trans_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE punishment_trans_uuid = $4 RETURNING *";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req) => {
  try {
    return (
      "SELECT * FROM vw_punishment_trans_table_dropdown " + mTools.getCondition(req)
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlEdit = (req) => {
  try {
    ...
    return (
      "UPDATE punishment_trans_table SET" +
      " ( " +
      keys +
      " ) = ( " +
      index +
      " ) " +
      "WHERE punishment_trans_uuid = $1 RETURNING *"
    );
  } catch (e) {
    throw new Error(e.message);
  }
...
let getSqlList = (req) => {
  try {
    return "SELECT * FROM vw_punishment_trans_table_list " + mTools.getCondition(req);
  } catch (e) {
    throw new Error(e.message);
  }
};
let getSqlView = (model) => {
  try {
    return "SELECT * FROM vw_punishment_trans_table_item WHERE punishment_trans_uuid = $1";
  } catch (e) {
    throw new Error(e.message);
  }
};
let getValuesCreate = (model) => {
  try {
    ...
    valuelist.push(data.punishment_type_uuid);
    valuelist.push(data.total_amount);
    valuelist.push(data.period);
    valuelist.push(data.amount_period);
    valuelist.push(mTools.toDateCurrectFormat(data.start_date));
    valuelist.push(mTools.toDateCurrectFormat(data.end_date));
...
let getValuesEdit = (model) => {
  try {
    ...
    let valuelist = [];
    valuelist.push(data.punishment_trans_uuid);
    valuelist.push(data.employee_uuid);
    valuelist.push(data.punishment_type_uuid);
    valuelist.push(data.total_amount);
    valuelist.push(data.period);
...
let toCreateModel = (req) => {
  try {
    ...
    model.punishment_type_uuid = req.body["punishment_type_uuid"];
    model.total_amount = req.body["total_amount"];
    model.period = req.body["period"];
    model.amount_period = req.body["amount_period"];
...
let toDropdown = (modelList) => {
  try {
    ...
    modelList.forEach((model) => {
      let value = model["punishment_trans_uuid"];
...
let getLabel = (model) => {
  try {
    return model["employee_uuid"] + " - " + model["punishment_type_uuid"];
  } catch (e) {
    throw new Error(e.message);
  }
};
let toEditModel = (req) => {
  try {
    const update_date = new Date().toISOString();
    var model = {};
    model.punishment_trans_uuid = req.body["punishment_trans_uuid"];
    model.employee_uuid = req.body["employee_uuid"];
    model.punishment_type_uuid = req.body["punishment_type_uuid"];
    model.total_amount = req.body["total_amount"];
    model.period = req.body["period"];
